...
int read_joy(int joy_id, int joy_conf[3]) {
  int out = map(raw_read_joy(joy_id)-joy_conf[0],joy_conf[1],joy_conf[2],-throttle_limit,throttle_limit);
...
void draw_display() {
  ...
  display.print("C:"); display.print(channel); display.print("L:"); display.println(throttle_limit); 
...
void loop()
{
  ...
  else
  {
    ...
    if(button_clicked(6)) {
      ...
      throttle_limit = min(32, throttle_limit + throttle_inc);
    }
    else if(button_clicked(1)) {
      throttle_limit = max(0, throttle_limit - throttle_inc);
    }
    draw_display();
  }
}
